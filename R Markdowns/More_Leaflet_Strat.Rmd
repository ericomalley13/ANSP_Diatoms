...
plotLeaflet_abun <- function(object, df_with_count_pct, title = "Taxa % Abundance", save_im=FALSE, path = "") {
            ...
            symbols <- makeSymbolIcons(values = df_with_count_pct$count_pct,
                                     shape = 'diamond',
                                     width = df_with_count_pct$count_pct,
                                     color = 'black',
                                     pal = numPal,
                                     opacity = .5,
                                     baseSize = 4)
...
plotLeaflet_abun(taxa_dl, df_with_count_pct = taxa_abun, title = "E. exigua % abundance")
```

```{r,trying to figure out issue}
df1 <- map(taxa_dl@sites, function(x) {
              df <- data.frame(siteid = x@siteid,
                               sitename = x@sitename,
                               lat = mean(st_coordinates(x@geography)[, 2]),
                               long = mean(st_coordinates(x@geography)[, 1]),
              ...
              bind_rows()
            numPal <- colorNumeric('viridis', taxa_abun$count_pct)
            symbols <- makeSymbolsSize(values = taxa_abun$count_pct,
                                     shape = 'diamond',
                                     color = 'black',
                                     fillColor = ~numPal(taxa_abun),
                                     opacity = .5,
                                     baseSize = 4)
            map1 <- leaflet(df1) %>%
              addProviderTiles(providers$Stamen.TerrainBackground) %>%
