...
##################################################################################
wc_plot_2 <- function(wc_ids,diat_ids,taxa_name, title = "Phosphorus") {
  wc_samp <- get_datasets(ALL_WC) %>%
    get_downloads() %>%
    samples()
  ...
  diat_abun <- get_datasets(ALL_DIAT) %>%
    get_downloads() %>%
    pc_abun()
  ...
  diat_taxa <- arrange(diat_taxa,siteid)
  diat_taxa_1 <- aggregate(diat_taxa$count_pct ~ diat_taxa$siteid + diat_taxa$lat + diat_taxa$long, data = diat_taxa, FUN = mean)
  
  test <- subset(wc_phos_df, wc_phos_df$siteid %in% diat_taxa$siteid)
  test2 <- subset(diat_taxa, diat_taxa$siteid %in% test$siteid)
  test3 <- aggregate(test2$count_pct ~ test2$siteid + test2$lat + test2$long, data = test2, FUN = mean)
  ...
  phos_vs_abun_lir <- plot(x_ph,y_abun, xlab = "Total Phosphorus (µg/L)", ylab = "% Abundance", main = "Aulacoseira lirata vs. Phosphorus")
  ...
  # wc_ph_2 <- wc_ph %>%
  #   dplyr::filter(elementtype == "field" | is.na(wc_ph$elementtype))
  ...
  phos_vs_abun_lir <- plot(x_ph,y_abun, xlab = "Total Phosphorus (µg/L)", ylab = "% Abundance", main = "Aulacoseira lirata vs. Phosphorus")
  ##################################################
  ##########MAKE THIS A FUNCTION: LEFT OFF 5/10/22 4:08
